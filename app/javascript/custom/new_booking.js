document.addEventListener("DOMContentLoaded", event => {
  ...
  form.addEventListener('input', event => {
    const ownerFirstNameValue = document.getElementById("booking_owner_first_name").value
    const ownerLastNameValue = document.getElementById("booking_owner_last_name").value
    const petNameValue = document.getElementById("booking_pet_name").value
    const petTypeValue = document.getElementById("booking_pet_type_id").value
    ...
    if (ownerFirstNameValue && ownerLastNameValue && petNameValue && petTypeValue && durationValue && dateValue) {
  ...
  const priceChangeHandler = async event => {
    const pet_type = parseInt(document.getElementById("booking_pet_type_id").value)
    const duration = parseInt(document.getElementById("booking_duration").value)

    if (pet_type && duration) {
      const resp = await fetch(`/pricing_engine/get_price?pet_type=${pet_type}&duration=${duration}`)
